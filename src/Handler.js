...
module.exports = class Handler {
    constructor(options) {
		...
		if (this.options.crossOrigin) {
			var isJs = /(\.js)$/.test(this.options.fileName)
			// 跨域时强制将文件改为js文件
			if (!isJs) {
				var exts = this.options.fileName.split('.')
				exts.pop()
				exts.push('js')
				this.options.fileName = exts.join('.')
			}
		}
    ...
    handle(compilation) {
		...
		var isJsFile = /(\.js)$/.test(this.options.fileName)
		if (isJsFile) {
		...
		// 跨域要采用jsonp获取数据，提前封装
		if (isJsFile) {
			output = `function getThemeColorCss() { return "${output}" }`
		}
    ...
    // 自动注入js代码，设置css文件名
    addToEntryJs(outputName, compilation, output) {
        var onlyEntrypoints = {
            entrypoints: true,
            errorDetails: false,
            modules: false,
            ...
            for (var i = 0, l = entryAssets.length; i < l; i++) {
                ...
                if (assetName.slice(-3) === '.js' && assetName.indexOf('manifest.') === -1) {
                    ...
                    if (assetCode && !assetCode._isThemeJsInjected) {
                        var config = {url: outputName, colors: this.options.matchColors, publicPath: compilation.outputOptions.publicPath}
                        var cSrc = new ConcatSource(
                            `window.__theme_COLOR_cfg=${JSON.stringify(config)};`,
							'\n',
							`window.DocerThemeStyles = (${output})();`,
							'\n',
                            assetCode,
                        );
